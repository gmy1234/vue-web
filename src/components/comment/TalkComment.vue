        ...
        <div class="comment-meta">
          ...
          <div class="comment-info">
            ...
            <span @click="like(item)">
              <v-icon :class="isLike(item.id)">mdi-thumb-up</v-icon>
            </span>
...
export default {
  ...
  methods: {
    ...
    listComments() {
      ...
      const param = {current: this.current, type: this.type};
      switch (this.type) {
        case 1:
        case 3:
          param.topicId = arr[2];
      ...
      this.axios.get("/api/comment/getComments", {params: param}).then(res => {
        if (this.current === 1) {
          this.commentList = res.data.data.recordList;
        } else {
          this.commentList.push(...res.data.data.recordList);
        }
        this.current++;
